...
public class TopBySubscribersCommand extends VkCommand {

    public TopBySubscribersCommand(Integer vkId) {
        super(vkId);
    }
    ...
    public void run() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            msg += (i+1) + ". @id" + users.get(i).getVkId() + " (" + user.get(0).getFirstName() + ")" +
                    " - " + users.get(i).getDetails_id().getSubscribers() + "\uD83D\uDC6B\n";
        }
        sendMessage(msg, MyKeyboards.getSubscribersTopKeyboard());
    }
}
